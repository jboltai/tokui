...
| `agent.*` / `toolCall.*` / `artifact.*` / `testResult.*` etc. | `agent.defaultName` / `toolCall.defaultName` / `artifact.codeTab` / `testResult.total` | 智能体 / 工具 / 代码 / 总计：{n} | Agent / tool / Code / Total: {n} |
