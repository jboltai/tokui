...
The Python SDK will provide a chained API aligned with the Node.js [`TokUIBuilder`](./builder) for generating TokUI DSL strings on a Python backend, paired with SSE streaming to the frontend.
...
- `to_string()` one-shot output, `to_chunks()` chunked list
