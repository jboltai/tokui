    ...
    build() {
      ...
      b.h2('Command 命令面板')
        ...
        .command({ ph: '输入工具名称...', id: 'cmdMini' })
          ...
          .end()
        .end()
        .h3('根级回调（command 带 clk）')
        .p('command 根设 clk 后，选中任意命令项时除项级回调外还会触发根级回调（payload 带项的 value/text/clk）；点击面板空白/搜索框不触发。')
        .btn({ tx: '打开项目面板', clk: 'openCommand', 'data-target': 'cmdRoot' })
        .command({ ph: '搜索项目命令...', id: 'cmdRoot', clk: 'cmdRootSelect' })
          .commandGroup({ tt: '项目' })
            .item({ tx: '新建项目', clk: 'newProject' })
            .item({ tx: '项目设置', clk: 'moreSettings' })
          .end()
        .end();
      return b;
    }
